// cc -o trfs trfs.c `pkg-config --cflags --libs fuse` -lulockmgr -ggdb -g3 -std=gnu11
// ./trfs <rootdir> <mountdir> [options]

// translates characters such as spaces into alternative chars
// this is mainly for the acme text editor, since it can't handle spaces
// or the like
...
int
fsgetattr(const char *path, struct stat *stbuf)
{
	char *fpath;
	int res;
...
int
fsfgetattr(const char *path, struct stat *stbuf,
...
int
fsreaddir(const char *path, void *buf, fuse_fill_dir_t filler,
          off_t offset, struct fuse_file_info *fi)
{
	...
	while (1) {
		...
		if (filler(buf, fpath, &st, nextoff))
			break;
...
int
fschmod(const char *path, mode_t mode)
{
	char *fpath;
	int res;
...
int
fschown(const char *path, uid_t uid, gid_t gid)
{
	char *fpath;
	int res;
...
int
fsutimens(const char *path, const struct timespec ts[2])
{
	int res;
...
int
fsread(const char *path, char *buf, size_t size, off_t offset,
       struct fuse_file_info *fi)
{
	int res;
...
int
fsreadbuf(const char *path, struct fuse_bufvec **bufp,
...
int
fswrite(const char *path, const char *buf, size_t size,
        off_t offset, struct fuse_file_info *fi)
{
	int res;
...
int
fswritebuf(const char *path, struct fuse_bufvec *buf,
           off_t offset, struct fuse_file_info *fi)
{
...
int
fsfsync(const char *path, int isdatasync,
        struct fuse_file_info *fi)
{
	int res;
...
int
fsfallocate(const char *path, int mode,
...
int fssetxattr(const char *path, const char *name, const char *value,
...
int
fsgetxattr(const char *path, const char *name, char *value,
           size_t size)
{
	char *fpath;
	int res;
...
int
fslistxattr(const char *path, char *list, size_t size)
{
	char *fpath;
	int res;
	...
	res = llistxattr(fpath, name, value, size);
	free(fpath);
	if (res < 0)
		return -errno;
	return res;
...
int
fsremovexattr(const char *path, const char *name)
{
	char *fpath;
	int res;
	...
	res = lremovexattr(fpath, name, value, size);
	free(fpath);
	if (res < 0)
		return -errno;
	return res;
...
int
fslock(const char *path, struct fuse_file_info *fi, int cmd,
       struct flock *lock)
{
	USED(path);
